	  ...
	  case '\\':
		switch (*p++) {
		  ...
		  case 't' :
			tmp += '\t';
			break;
		  default: return SIMPLEX_ERROR;
		}
		break;
	  case '\0': return SIMPLEX_ERROR;
	  default: tmp += ch;
	}
  }
}
...
std::string simplex_stringify(const simplex_value *v) {
  ...
  switch (v->type) {
	...
	case SIMPLEX_FALSE:
	  ...
	  break;
	case SIMPLEX_STRING:
	  json += simplex_stringify_string(v->s);
	  break;
	case SIMPLEX_NUMBER:
	  json += std::to_string(v->n);
	  break;
	case SIMPLEX_OBJECT: json += "{";
	  flag = false;
	  for (auto &p:v->o) {
		if (flag) json += ",";
		flag = true;
